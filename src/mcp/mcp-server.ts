        ...
        tools: [
          ...
          },
          {
            ...
            description: (() => {
              ...
              return `【禁止查表】在调用本工具之前，绝对禁止查询任何数据库表结构（如 get_schema、get_table_info、execute_query 等）、禁止检查连接状态、禁止询问用户"表名是什么""字段对不对"等。本工具是完整的黑盒业务接口，后台表映射、字段映射、ID 生成、数据库连接等全部细节已预配置完成，AI 只需要传入业务参数即可，无需关心任何底层实现细节。

【功能】向 Hzero 平台注册新的错误码及其多语言提示信息。当用户描述业务场景并提到"xxx时，报错xxx"、"报错xxxx"、"出现错误/异常/失败"、"需要抛出一个错误"、"新增错误码"、"消息维护"等情境时，AI 应主动生成合适的 MESSAGE_CODE 和 MESSAGE，并调用此工具完成注册。

【MESSAGE_CODE 生成规则】
- 格式：模块名.模块描述_递增编号（全大写）。模块名与模块描述之间用点号分隔，模块描述内部单词用下划线连接，模块描述与递增编号之间用下划线分隔。
...
- 示例：${locales.length === 3 ? '["连接超时", "Connection timeout", "Hết thởi gian chờ"]' : '["连接超时", "Connection timeout"]'}。缺失语言会自动回退到数组第一项。`;
            ...
            inputSchema: {
              ...
              properties: {
                data: {
                  type: 'array',
                  description: '错误信息数据列表（每行包含 MESSAGE_CODE 和 MESSAGE）',
                  items: {
                    type: 'object',
                    properties: {
                      MESSAGE_CODE: { type: 'string', description: '消息编码，必填' },
                      MESSAGE: {
                        anyOf: [
