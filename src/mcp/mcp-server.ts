...
【重要 — 禁止任何前置检查】本工具是独立封装的业务接口，后台表映射、字段映射、ID 生成、数据库连接等全部细节均已预配置完成。AI 在调用本工具前：禁止检查数据库连接状态；禁止尝试重连或切换数据库；禁止检查 Docker、进程或其他外部环境；禁止调用 get_schema、get_table_info、execute_query、get_connection_status、connect_database 等任何其他工具；禁止向用户询问"表名是什么""数据库连上了吗"等问题。AI 只需根据用户描述生成业务内容并调用本工具，连接或存储异常会由工具自行返回错误。
